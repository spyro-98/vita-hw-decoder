...
/* Hardware and software are deliberately shipped as different packages.
 * The hardware package must never silently move a workload back to the CPU. */
#ifndef VITA_HW_DECODER_ALLOW_SOFTWARE_FALLBACK
#define VITA_HW_DECODER_ALLOW_SOFTWARE_FALLBACK 0
#endif
...
#if VITA_HW_DECODER_HAS_H264_VITA
...
#if !VITA_HW_DECODER_ALLOW_SOFTWARE_FALLBACK
	(void)st;
	(void)stream;
	(void)packets;
	(void)count;
...
static void *vita_hw_software_video_thread(void *argp) {
...
#else
			if (!hardware_codec) init_ret = AVERROR_DECODER_NOT_FOUND;
#endif
		}
	}
	if (init_ret >= 0) {
...
static int allocate_copy_ring(SoftwareVideoState *st) {
	for (int i = 0; i < SOFTWARE_VIDEO_COPY_FRAME_SLOTS; i++) {
		char name[32];
		snprintf(name, sizeof(name), "VitaHwDecoderSwFrame%d", i);
...
static int allocate_zero_copy_pool(SoftwareVideoState *st) {
	for (int i = 0; i < SOFTWARE_VIDEO_FRAME_SLOTS; i++) {
	...
	for (int i = 0; i < SOFTWARE_VIDEO_ZERO_COPY_SURFACES; i++) {
		char name[32];
		snprintf(name, sizeof(name), "VitaHwDecoderZeroYuv%d", i);
...
int vita_hw_software_video_start(SoftwareVideoState *st, AVFormatContext *demux,
	...
	int hardware_prepare_ret = vita_hw_decoder_runtime_prepare();
	...
	if (ret == 0)
		ret = pthread_create(&st->thread, &attr, vita_hw_software_video_thread, st);
...
int vita_hw_software_video_present_rect(SoftwareVideoState *st,
...
int vita_hw_software_video_present(SoftwareVideoState *st,
	                       uint64_t presentation_clock_us,
	                       uint64_t raw_audio_clock_us,
	                       int fill_screen, int allow_advance) {
	return vita_hw_software_video_present_rect(
	    st, presentation_clock_us, raw_audio_clock_us,
	    0.0f, 0.0f, (float)SCREEN_WIDTH, (float)SCREEN_HEIGHT,
	    fill_screen, allow_advance);
}

void vita_hw_software_video_discard_to_clock(SoftwareVideoState *st,
...
void vita_hw_software_video_buffer_status(SoftwareVideoState *st, int *ready_frames,
	                              int *have_display,
...
void vita_hw_software_video_debug_snapshot(SoftwareVideoState *st,
...
void vita_hw_software_video_render_complete(SoftwareVideoState *st) {
	if (!st) return;
	slots_lock(st);
	if (st->retire_slot >= 0) {
...
void vita_hw_software_video_join(SoftwareVideoState *st) {
	if (!st) return;
	st->stop = 1;
	if (st->started) {
		int join_ret = pthread_join(st->thread, NULL);
