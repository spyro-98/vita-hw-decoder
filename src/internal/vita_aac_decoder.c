...
static void *allocate_uncached(uint32_t size, SceUID *out_memblock) {
	...
	SceUID memblock = sceKernelAllocMemBlock("VitaHwDecoderAacBuf",
...
int vita_hw_aac_decoder_init(VitaAacDecoder *dec, uint32_t channels,
	                      uint32_t sample_rate) {
	return vita_hw_aac_decoder_init_ex(dec, channels, sample_rate, 0);
}
